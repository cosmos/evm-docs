...
For developers who interface with account types on a Cosmos EVM chain,
e.g. during wallet integration on their dApp frontend,
it is therefore important to understand that accounts in the Cosmos EVM are implemented
...
Cosmos blockchains, like the Cosmos Hub, support creating accounts with mnemonic phrases,
otherwise known as
...
A common address form for Cosmos chains is the bech32 format (e.g. `cosmos1...`).
...
Cosmos EVM defines its own custom `Account` type
...
It is recommended to use the Coin type `60` to support Ethereum type accounts,
...
The custom Cosmos EVM [EthAccount](https://github.com/cosmos/evm/blob/main/types/account.go#L28-L33)
...
output addresses that contains a human-readable part that identifies the Bech32 usage.
...
`EthAccount` can be represented in both [Bech32](https://en.bitcoin.it/wiki/Bech32) (e.g. `cosmos1...`)
...
- **Address (Bech32)**: `cosmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw`
...
The `appd debug addr <address>` can be used to convert an address between hex and bech32 formats. For example:
...
```bash title="Hex"
 $ cosmosd debug addr 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Address: [20 87 74 109 255 45 223 158 7 130 139 67 69 211 4 9 25 175 86 82]
  Address (hex): 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Bech32 Acc: cosmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
  Bech32 Val: cosmosvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn
...
We can use the `keys show` command of `appd` with the flag `--bech <type> (acc|val|cons)` to
...
curl -X GET "http://localhost:10337/cosmos/auth/v1beta1/accounts/cosmos14au322k9munkmx5wrchz9q30juf5wjgz2cfqku" -H "accept: application/json"
```
